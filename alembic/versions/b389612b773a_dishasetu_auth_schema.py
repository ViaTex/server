...
def upgrade() -> None:
    ...
    if not conn.execute(sa.text("SELECT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'userstatus')")).scalar():
        op.execute("CREATE TYPE userstatus AS ENUM ('ACTIVE', 'INACTIVE', 'SUSPENDED', 'PENDING');")
        
    if not conn.execute(sa.text("SELECT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'gender')")).scalar():
        op.execute("CREATE TYPE gender AS ENUM ('MALE', 'FEMALE', 'OTHER');")
        
    if not conn.execute(sa.text("SELECT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'usertype')")).scalar():
